...
const CoursePage: React.FC<CoursePageProps> = memo(({ selectedCategory }) => {
  ...
  const filteredCourses = useMemo(() => {
    // If selectedCategory is null, show all courses
  ...
  // Don't render anything if selectedCategory is still null (loading state)
  ...
  return (
    <motion.div
      key={selectedCategory} // Add key to ensure proper re-animation when category changes
    ...
    >
      {filteredCourses.map((course, index) => (
        <motion.div
          key={`${selectedCategory}-${course.id}`} // Unique key to prevent conflicts
